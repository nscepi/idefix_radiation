...
Equations
---------
The radiation module treats the photon field as an effective radiative fluid by using the M1 approximation.  grains as a zero-pressure gas coupled to the gas by a linear drag force on the velocity.
...
.. math::
    &\partial_t \rho + \nabla \cdot \left[ \rho \mathbf{v} \right] = 0,\label{eq:continuity} \\
    &\partial_t (\rho \mathbf{v}) + \nabla \cdot \left[ \rho \mathbf{v} \otimes \mathbf{v} - \mathbf{B} \otimes \mathbf{B} + \mathcal{P} \mathbb{I} \right] = -\rho \nabla \psi + \mathbf{G}, \\
    &\partial_t \mathbf{B} + \nabla \times \left[ - \mathbf{v} \times \mathbf{B} \right] = 0, \\
    &\partial_t E + \nabla \cdot \left[ (E + \mathcal{P}) \mathbf{v} - \mathbf{B}(\mathbf{B} \cdot \mathbf{v}) \right]  \nonumber\\
    &\phantom{\partial_t E + \nabla \cdot [} \left. = -\rho \mathbf{v} \cdot \nabla \psi + c G^0 - \nabla \cdot \mathbf{F}_\mathrm{irr},\label{eq:energy}\\
    &\partial_t E_r + \nabla \cdot \mathbf{F}_r = -\hat{c}\:G^0,\label{eq:rad1} \\
    &\partial_t \mathbf{F}_r + \nabla \cdot \mathbb{P}_r = - \hat{c}\:\mathbf{G}, \label{eq:rad2}
