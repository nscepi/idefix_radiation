    ...
    for(int k = 0; k < d.np_tot[KDIR] ; k++) {
        for(int j = 0; j < d.np_tot[JDIR] ; j++) {
            for(int i = 0; i < d.np_tot[IDIR] ; i++) {
              d.Vc(RHO,k,j,i) = 7.78e-10/unit_density;
              //d.Vc(RHO,k,j,i) = 1.e-7/unit_density;
              d.Vc(PRS,k,j,i) = d.Vc(RHO,k,j,i)*unit_density*KELVIN*T/(unit_energy);
              //d.Vc(PRS,k,j,i) = d.Vc(RHO,k,j,i)*unit_density*KELVIN*T/(unit_energy);
              d.Vc(VX1,k,j,i) = -20.0e5/unit_velocity;
              //d.Vc(VX1,k,j,i) = ZERO_F;
              d.RadVc[0](ER,k,j,i) = C_ar*std::pow(T,4)/(unit_energy);
              //d.RadVc[0](ER,k,j,i) = 1.e4/unit_energy;
